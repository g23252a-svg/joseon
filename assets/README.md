...
여기에 PNG·WebP 에셋을 놓으면 게임이 자동으로 씁니다.
**규격은 상위 폴더의 [`ASSETS.md`](../ASSETS.md) 를 보십시오.**

| 파일 | 최종 크기 | 한 칸 | 상태 |
|---|---|---|---|
| `map_geo.js` | 약 125KB | 26개 Path2D 권역 | ☑ |
| `east_asia_relief.webp` | 968×572 | 실제 지표·음영기복 | ☑ |
...
`east_asia_relief.webp`는 Natural Earth II 3.2.0 공개 도메인 지형 래스터를
같은 투영으로 재표본화한 결과입니다. 원본 TIFF는 저장소에 넣지 않으며,
`tools/generate_relief.py --admin1 <원본.geojson> --raster <원본.tif> --out assets/east_asia_relief.webp`로 재생성합니다.
